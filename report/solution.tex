...
The first approach to distribute the calculation over the threads in the GPU, was just to let each thread do the calculations for one singel face-face pair, to the corresponding vertex-face and edge-edge pairs. We would assign 1024 threads per block, in a 16x16 matrix, and then have the number of blocks calculated as follows: for the x-direction we used \texttt{nFaces / threadsPerBlock.x} and in the y-direction \texttt{maxSize / threadsPerBlock.y}. This way we have \texttt{nFaces * maxSize} threads, which corresponds to the maximum amount of potential face-face pairs. \\
%However, this approach could spawn too many threads, in which case we need to do more calculations per thread.
